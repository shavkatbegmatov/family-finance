...
--
-- IDEMPOTENT: barcha CREATE va ALTER statement'lar IF NOT EXISTS bilan,
-- shuning uchun qisman bajarilgan oldingi urinish ham xato bermaydi.
-- =====================================================
...
ALTER TABLE accounts ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE family_members ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE point_configs ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE point_participants ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE point_balances ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE point_savings_accounts ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE point_challenges ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;

-- =====================================================
-- 2) family_group_id BO'LMAGAN entity'lar (bug fix)
-- =====================================================

ALTER TABLE budgets ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE debts ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
ALTER TABLE savings_goals ADD COLUMN IF NOT EXISTS scope_id BIGINT REFERENCES scopes(id) ON DELETE RESTRICT;
...
-- WHERE scope_id IS NULL — agar oldingi urinish to'ldirgan bo'lsa qayta yozmaslik
...
-- =====================================================
-- 5) Indekslar — query performance uchun (idempotent)
-- =====================================================

CREATE INDEX IF NOT EXISTS idx_accounts_scope ON accounts(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_family_members_scope ON family_members(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_budgets_scope ON budgets(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_debts_scope ON debts(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_savings_goals_scope ON savings_goals(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_point_configs_scope ON point_configs(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_point_participants_scope ON point_participants(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_point_balances_scope ON point_balances(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_point_savings_accounts_scope ON point_savings_accounts(scope_id) WHERE scope_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_point_challenges_scope ON point_challenges(scope_id) WHERE scope_id IS NOT NULL;
