...
to one or more *Households* (xonadon, mustaqil root) and education contexts
(*School*/*Class*). `GROUP` is retired and kept only for archived/legacy rows; project/event/fund
style financial contexts are deprecated and creation is blocked. Genealogiya (qarindoshlik grafi)
moliyadan ajratilgan — ADR-001 (`docs/adr-001-genealogy-finance-decoupling.md`), GROUP removal is
ADR-003 (`docs/adr-003-remove-group-scope.md`), and schools/classes are ADR-002. Monorepo:
...
> **Automated tests:** Backend JUnit5 unit tests plus Testcontainers integration
> (`*IntegrationTest` — real PG, CI'da). Frontend Vitest (`src/utils/*.test.ts`) and Playwright
> smoke (`e2e/smoke.spec.ts`). "Verify" = `./mvnw clean compile`/`test`,
> `npm run build`/`npm run lint`/`npm run test`/`npm run e2e`, + **manual preview** (front :5178 ↔ back :8098,
...
- **Backend:** DTOs are `*Request` / `*Response` under `dto/request` & `dto/response`.
  ...
  binds `bytea` → `lower(bytea)` error). New schema = a new Flyway migration after the latest
  existing `V*` file (never edit applied ones).
